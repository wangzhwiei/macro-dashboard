...
## 20. iFinD 同日缓存与北京二手房频率修复（2026-08-07）

### 性能修复

原缓存只保存历史 `records`，没有记录“今天已检查但无新增”。因此 08:30 主 run、09:10 兜底和人工重跑会在同一天重复查询 41 个 iFinD 序列；每个无新增结果此前还会重试 3 次。

已改为兼容旧格式的缓存对象：

```json
{"records": [], "last_checked_date": "2026-08-07"}
```

- 同一天已成功检查的序列直接返回缓存。
- 供应商明确返回“未返回可用数据”时只查询一次并记录当天。
- 网络、认证等真正异常仍最多重试 3 次，且不写成功检查日期。
- provider ID 漂移立即失败，不重试、不静默退回旧缓存。

首轮 41 个缓存格式升级的 Actions 抓取耗时 10 分 22 秒；升级后本机真实完整 data-only pipeline 耗时 46.6 秒，严格校验和 26 项测试均通过。 最终发布 run `31145467996` 中 `Fetch data` 实测 38 秒，`update-and-build` 与 `deploy` 均成功；线上 `generatedAt=2026-08-07T03:50:59.325506+00:00`。

### 北京二手房组件修复

`IFIND:SECONDHAND_BEIJING_UNITS` 旧配置锁定 `S003059022`，本地缓存实际为月频：43 个点，最新 `2026-06-30`。iFinD 自然语言匹配会随日期区间漂移，短区间曾返回其他周频 provider。

经过长短区间和候选名称核验，最终配置为：

- 查询名：`北京:二手房:销售套数:周`
- provider ID：`S004368947`
- 频率：`W`
