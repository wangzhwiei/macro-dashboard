...
每个指标以最近一周的零变化为方向中轴，并用自身历史周变化的均方根波动把强度标准化到 `-100～100`。历史波动只影响强度，不会反转实际涨跌对应的债市方向：
...
中性项不进入分母。大类卡片按核心指标族计票，而不是按原始指标条数计票，避免同一产业链、期限或地区的多个指标重复放大某一方向；综合扩散度则按九个大类计票。扩散度表示观点的覆盖面，不等同于信号强度。

历史观点与趋势矩阵从 `2023-01-01` 起按每周五生成完整序列，并随每日更新持续向后追加。
